...
In a generalized linear model (GLM), we specify 2 things:

1. Response likelihood (“family”)
  - e.g., Normal (“Gaussian”), Binomial, Poisson, etc.
  - Until now, we’ve used the default family = "gaussian" (Normal)
  - Now we are changing it up!
    - e.g., family = "binomial" or  family = binomial(link = "logit")

-Linear models for each distribution parameter
  - e.g., 𝜇𝑖 = 𝛽0 +𝛽_1 𝑥_1𝑖+𝛽_2 𝑥_2𝑖 ; log(𝜎_𝑖 )=𝛾_0+𝛾_1 𝑥_1𝑖
---
# Fitting a Binomial Regression in R
...
---
class: middle center
# What the fuck are likelihoods and how do they apply to logistic regression?
...
tidy(fit,exponentiate = TRUE) # odds ratio
...
- Intercept
  ...
  - When budget = 0, log-odds of Y are 0.11
  
    -  A movie with a budget of zero has a .11 log of passing the test
...
- As $X_i$ increases by 1 unit, log-odds of Y increases by β1

- As $X_i$ increases by 1 unit, the odds of Y multiply by a factor of  exp{β1}
...
- If X is a categorical predictor
  ...
  - The difference in the log-odds between group X and the baseline is β1
...
- `glm()` two deviances are calculated: 
  ...
  1. Residual deviance (model deviance)
    - Measure of how much the observed data differs from what is measured using the likelihood function
