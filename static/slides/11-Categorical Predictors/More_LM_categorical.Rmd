...
- Categorical predictors
  ...
  - Contrast coding for two means 
    
  - Contrast coding > 2 means
---
# Check-in Questions
...
- Consequences:
  ...
  - Causes stahdard errors (SE) to be unreliable
...
- Find line using least squares regression
...
senses_filt %>%
  ggplot(aes(x=Modality["Taste"], y = Val)) + 
   geom_point2(size = 4, alpha = 0.3) + 
   stat_summary(fun.y = mean, color = "black", geom ="point", aes(group = 1), size = 9,
...
senses_filt %>%
  ggplot(aes(x=Modality["Smell"], y = Val)) + 
   geom_point2(size = 4, alpha = 0.3) + 
   stat_summary(fun.y = mean, color = "black", geom ="point", aes(group = 1), size = 9,
                   show.legend = FALSE, color="red") + 
  stat_summary(fun="mean", geom="segment", mapping=aes(xend=..x.. - 0.25, yend=..y..), color="red", size=3) + 
  stat_summary(fun="mean", geom="segment", mapping=aes(xend=..x.. + 0.25, yend=..y..), color="red", size=3)
```
]
---
...
- Mean of Smell: $\hat\beta_0+\hat\beta_1\times .5=5.64-0.33=5.47$

- Mean of Taste: $\hat\beta_0+\hat\beta_1\times -.5=5.64-0.33=5.80$

---
# Default R behavior

- Change levels from (0-1) to + (1 and -1)
...
- Intercept is now centered at 0
...
- Tells us if overall model is significant fit (difference between group means is 0)

$$\begin{aligned} &&df_{R} = N - 1\\  &&df_{F} = N - a\\ &&SSE(R)=SS_{total}\\&&SSE(F)=SS_{error}\end{aligned}$$
$$F^*=\left( \dfrac{SS_{total}-SS_{error}}{(df_R)-(df_F)}\right)\div\left( \dfrac{SS_{model}}{df_F}\right)=\frac{MSR}{MSE}$$
...
3. Dummy code the unemployment variable

5. Run `lm` on the dummy coded variable

6. Interpret the output 

6. Run `lm` on the contrast coded (.5, -.5) variable

7. Interpret the outputs 

8. Use output to extract the means for each group only using the output. Are they similar to what you hand coded?
...
2. Choose one of your levels as baseline (default to 0 for whatever comes first alphabetically)

3. Assign all zeros for this level across each dummy code

4. For first dummy code, assign 1 to first group and 0s for rest of levels

5. For the second dummy code, assign 1 to second group and 0s for rest of levels

6. For third dummy code, assign 1 to third group and 0s for rest of levels

7. For fourth dummy code, assign 1 to fourth group and 0s for rest of levels
...
```{r}
sense_ml <- lm(Val~Modality, data=senses)
```
...
knitr::include_graphics("part.bmp")
```
---
# The General Linear F-Test

- Useful if you have more than 2 levels for a factor

- Test overall differences between the groups *F*-test
 
- We can think about the hypotheses for the overall test being:

$$\operatorname{H_0} =  \beta_{1}(\operatorname{Modality}_{\operatorname{Smell}})= \beta_{2}(\operatorname{Modality}_{\operatorname{Sound}}) = \beta_{3}(\operatorname{Modality}_{\operatorname{Taste}}) = \beta_{4}(\operatorname{Modality}_{\operatorname{Touch}}) = \epsilon$$
...
- F = MSR / MSE (Same Mean Squared Error on ANOVA table outputs)
...
flextable(as.data.frame(means1))
...
Assumptions for linear modeling still apply
