...
Using the cloak data, calculate *d* using all three effect size measures
...
- Do invisibility cloaks effect behavior?
...
75% chance we’ll get a significant result when we do this study in the long run**
...
bilinguals, there is a 35% chance we’ll get p < .05 over the long-run**
---
# Why do we care?
...
- Understand null results
  ...
  - With high power, null result is more informative
      ...
      -  e.g., null effect of working memory training on intelligence with 20% power
...
3. Pilot data
  ...
  - Estimate effect size using current data (set α, desired power)
...
d2abs = .2
pwr.t.test(n= NULL, d = d2abs, power = 0.80, sig.level = 0.05,
...
---
```{r}
pwr.t.test(d = d1$Cohens_d, power = 0.80, sig.level = 0.05, type = "one.sample", alternative = "two.sided")$n
```
--
# Power Question
...
- Simulation based power (t.tests, ANOVA)

- https://cran.r-project.org/web/packages/Superpower/vignettes/intro_to_superpower.html#power-in-a-two-group-one-way-anova
---
# Simulation
